...
\begin{document}
	...
	%todo quicksort, heap, insertion, selection, merge, etc.
	
	%todo insert insertion sort description
...
\end{figure}
	...
	%todo The reasoning for this is, recursive functions have an overhead associated with them caused by the increased call stack \footnote{local variables must be reinitialized at each recursion level and the entrance of each function call must be stored so it can be returned to when the function exits}
...
\begin{thebibliography}{9}
	...
	Wikipedia–$\mbox{https://en.wikipedia.org/wiki/Call_stack}$
	
	\bibitem{wiki-recursiveFunction}
	Wikipedia–$\mbox{https://en.wikipedia.org/wiki/Recursive_function}$
